...
class Program:
    ...
    def user_choose_movie(self, movies):
        ...
        while True:
            try:
                ...
                if movie_index <= (index) and movie_index > 0:
                    breaking = True
                    while True:
                        answer = input('Are you sure that you want to go with {}? (Type y/n): '.format(movies_names[movie_index - 1]))
                        if answer == 'y':
                            break
                        if answer == 'n':
                            breaking = False
        ...
        while answer != 'y' and answer != 'n':
            answer = input('Do you want to get another movie from this list?(Type y/n): ')
        if answer == 'y': self.user_choose_movie(movies)
        else: self.try_to_start_program_again()
    ...
    def user_choose_serie(self, series):
        ...
        while True:
            try:
                ...
                if series_index <= (index) and series_index > 0:
                    breaking = True
                    while True:
                        answer = input('Are you sure that you want to go with {}? (Type y/n): '.format(series_names[series_index - 1]))
                        if answer == 'y':
                            break
                        if answer == 'n':
                            breaking = False
        ...
        while answer != 'y' and answer != 'n':
            answer = input('Do you want to get another movie from this list?(Type y/n): ')
        if answer == 'y': self.user_choose_serie(series)
        else: self.try_to_start_program_again()
        ...
        for href in serie_hrefs:
            ...
            for result_site in result:
                ...
                for season in result_site:
                   try: streaming_links[season]
                   except: streaming_links[season] = []

                   for episode_num in range(len(result_site[season])):
                       for streaming_link in result_site[season][episode_num]:
                           try: streaming_links[season][episode_num].append(streaming_link)
                           except: streaming_links[season].append([streaming_link])
