...
class Monitorer_directly:
    ...
    def main(self):
        print('Please type there the link of post (in your wp site) that you want to update (for example: https://test1.pelisya.io/movies/tom-clancy-sin-remordimientos-2021/)')
        link = input()
        print('Is it movie? (y/n)')
        movie = input()
        if movie != 'y' and movie != 'n': return
        if movie == 'y': movie = True
        else: movie = False
        ...
        post_id, post_title,season_num,episode_num = response['id'], response['title'], response['season_num'], response['episode_num']
        ...
        if movie:
            ...
            for movie_obj in [*Cuevana_scrapper().search_movie(movie_name), *Gnula_scrapper().search_movie(movie_name), *Pelisplushd_scrapper().search_movie(movie_name), *Pelisplus_scrapper().search_movie(movie_name)]:
                if movie_obj['title'].lower().replace(' ', '') in movie_name.lower().replace(' ', ''): the_same_movie_links.append(movie_obj['href'])

            for href in the_same_movie_links:
                for streaming_link in [*Cuevana_scrapper().scrape_movie(href), *Gnula_scrapper().scrape_movie(href), *Pelisplushd_scrapper().scrape_movie(href), *Pelisplus_scrapper().scrape_movie(href)]:
            ...
            print('Trying to find the need streaming links for the {} season {} episode {}...'.format(serie_name, season_num, episode_num))


            serie_hrefs = []
            for serie_obj in [*Cuevana_scrapper().search_series(serie_name), *Gnula_scrapper().search_series(serie_name), *Pelisplushd_scrapper().search_series(serie_name), *Pelisplus_scrapper().search_series(serie_name)]:
                if serie_obj['title'].lower().replace(' ', '') in serie_name.lower().replace(' ', ''): serie_hrefs.append(serie_obj['href'])

            serie_hrefs = serie_hrefs[0:1]
            #for different sites add streaming links for each episode
            streaming_links = {}
            for href in serie_hrefs:
                result = [Cuevana_scrapper().scrape_series(href), Gnula_scrapper().scrape_series(href), Pelisplushd_scrapper().scrape_series(href), Pelisplus_scrapper().scrape_series(href)]
            ...
            if streaming_links:
                try:
                    for stream_link in streaming_links[str(season_num)][int(episode_num) - 1]: stream_links.append(stream_link)
                except: pass
